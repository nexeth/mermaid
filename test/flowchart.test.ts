...
m((("double-circle")))`);
  ...
  describe("subgraph", () => {
    test("should be able to add a subgraph", () => {
      const flowchart = new Flowchart({ flowchartType: "TD" });
      flowchart.subgraph("a");
      expect(flowchart.flowchart).toEqual([{ type: "subgraph", id: "a" }]);
    });

    test("should be able to render a subgraph", () => {
      const flowchart = new Flowchart({ flowchartType: "TD" });
      expect(flowchart.renderSubgraph({ type: "subgraph", id: "a" })).toEqual(`subgraph a`);
    });

    test("should be able to render a subgraph with title", () => {
      const flowchart = new Flowchart({ flowchartType: "TD" });
      expect(flowchart.renderSubgraph({ type: "subgraph", id: "a", title: "title" })).toEqual(`subgraph a ["title"]`);
    });

    test("should be able to end a subgraph", () => {
      const flowchart = new Flowchart({ flowchartType: "TD" });
      flowchart.subgraph("a");
      flowchart.end();
      expect(flowchart.flowchart).toEqual([{ type: "subgraph", id: "a" }, { type: "end" }]);
    });

    test("should be able to alter the subgraph direction", () => {
      const flowchart = new Flowchart({ flowchartType: "TD" });
      flowchart.subgraph("a");
